...
public class VerticalProjectController {
    ...
    @RequestMapping("/getVerticalProject")
    @ResponseBody
    public void getVerticalProjectByOrganization() {
        List<SearchCount> x = verticalProjectService.findVerticalProjectQuantity("计算机科学与技术学院");
        if(!x.equals(null))
    ...
    @RequestMapping("/getVerticalProjectEveryYear")
    @ResponseBody
    public JSON getVerticalProjectByOrganizationNameEveryYear(String organizationName) {
        List<SearchCountByYear> x = verticalProjectService.findVerticalProjectQuantityByOrganizationNameAndEveryYear(organizationName);
        for(SearchCountByYear o: x) {
            if(o.getYear().length() < 4) {
                continue;
            }
